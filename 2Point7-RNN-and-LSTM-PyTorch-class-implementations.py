...
def evaluate(model, iterator, criterion, raw_texts):
    sample_index = 0
...
def iterator(X, y, shuffle=True):
    ...
    for i in range(0, size, batch_size):
        ...
        batch = {}
        batch["text"] = [X[i] for i in indices if len(X[i]) > 0]
        batch["label"] = [y[i] for i in indices if len(X[i]) > 0]
...
train_iterator = iterator(X_train, y_train, shuffle=True)
validate_iterator = iterator(X_val, y_val, shuffle=False)
test_iterator = iterator(X_test, y_test, shuffle=False)
...
for model in [rnn_model, lstm_model]:
    ...
    for epoch in range(N_EPOCHS):
        ...
        valid_loss, valid_acc, tracked_attention = evaluate(
            model,
            validate_iterator,
            rnn_criterion if isinstance(model, RNN) else lstm_criterion,
            raw_val
        )
...
# =========================
    ...
    print(f"\nif valid_loss < best_valid_loss:🏆 Best epoch: {best_epoch+1}")
